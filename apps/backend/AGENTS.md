...
`npm audit` reports dozens of vulnerabilities. **Nearly all are transitive
through `@medusajs/*` and cannot be fixed without upgrading Medusa itself.**
`npm audit fix --force` would install `vite@8` and a `@medusajs/test-utils`
