...
- [x] 1.1 Add `"@medusajs/js-sdk": "2.18.0"` and `"@medusajs/icons": "2.18.0"` to
...
- [x] 2.1 Create `apps/backend/src/admin/lib/sdk.ts` exporting one configured
...
- [x] 2.2 In the same file, export a `Supplier` type with exactly the fields the
...
- [x] 3.1 Create `apps/backend/src/admin/routes/suppliers/page.tsx` with a
...
- [x] 3.2 In that component, load the list with
...
- [x] 3.3 Add the four render states as early returns, in this order:
...
- [x] 3.4 Render the loaded state as a `@medusajs/ui` `Table` with a header row
...
- [x] 3.5 Render each supplier's name inside a `Link` from `react-router-dom`
...
- [x] 3.6 Style the link so it reads as one: `className="text-ui-fg-interactive
      hover:underline"` on the `Link`. Uses Medusa's own interactive-foreground
      token rather than a hand-picked colour, so it follows the dashboard theme.
      Verify: the name is visibly different from the email and phone cells in
      the same row (spec: *The link looks like a link*).

## 4. The detail page

- [x] 4.1 Create `apps/backend/src/admin/routes/suppliers/[id]/page.tsx` with a
...
- [x] 4.2 Load the supplier with
      ...
      mount.
- [x] 4.3 Add three render states as early returns: `isPending` → `Spinner`;
...
- [x] 4.4 Render the four fields — name, email, phone, collection address — as
...
- [x] 5.1 Run `npm run lint` from the repo root and fix anything it reports.
...
- [x] 5.2 Run `npm run build` in `apps/backend` and verify it completes with no
...
- [x] 6.1 **Empty state.** The database already held a supplier, so the empty
      state was verified in code instead: replacing `const suppliers =
      data.suppliers` with `const suppliers: Supplier[] = []` renders "There are
      no suppliers yet." with no table header row (spec: *No suppliers exist*).
      Reverted afterwards.
- [x] 6.2 **Loading state.** In devtools, set network throttling to a slow
...
- [x] 6.3 **Error state.** In devtools block the
...
- [x] 6.5 **Loaded list.** Reload `/app/suppliers`: a header row plus exactly
      two data rows, each showing that supplier's name, email and phone
      (spec: *Suppliers exist*).
- [x] 6.6 **No token anywhere.** In step 6.4 the `POST /admin/suppliers` response
      ...
      back nowhere.
- [x] 6.7 **Row navigation.** Tab to the first supplier's name link and press
...
- [x] 6.8 **Detail fields.** The detail page for the first supplier shows its
...
- [x] 6.9 **Detail loading and error states.** Repeat 6.2 and 6.3 against a
      detail page URL (spec: *Request in flight* / *Request fails*, detail).
- [x] 6.10 **Unknown supplier.** Open `/app/suppliers/sup_does_not_exist`. The
